<script lang="ts">
	...
	import { Eye, EyeOff, LoaderCircle, ArrowLeft } from '@lucide/svelte';
...
<div class="w-full max-w-[380px]">
	...
	<p class="mt-2 text-[14.5px] text-ink-500">{t('auth.adminLoginSubtitle')}</p>
	...
	<p class="mt-7 text-center text-[14px] text-ink-500">
		<button
			type="button"
			onclick={() => goto(`${base}/`)}
			class="inline-flex items-center gap-1 font-medium text-iris-600 hover:text-iris-700"
		>
			<ArrowLeft class="h-3.5 w-3.5" />
			{t('auth.backToStudentLogin')}
		</button>
	</p>
</div>
